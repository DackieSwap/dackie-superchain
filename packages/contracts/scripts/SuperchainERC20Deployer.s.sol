...
import {DackieSuperchain} from "../src/DackieSuperchain.sol";
...
contract SuperchainERC20Deployer is Script {
    ...
    function run() public {
        ...
        for (uint256 i = 0; i < chainsToDeployTo.length; i++) {
            ...
            (address _deployedAddress, address _ownerAddr) = deployDackieSuperchain();
    ...
    function deployDackieSuperchain() public broadcast returns (address addr_, address ownerAddr_) {
        ...
        bytes memory initCode = abi.encodePacked(
            type(DackieSuperchain).creationCode, abi.encode(ownerAddr_, name, symbol, uint8(decimals))
        );
        address preComputedAddress = vm.computeCreate2Address(_implSalt(), keccak256(initCode));
        if (preComputedAddress.code.length > 0) {
            console.log(
                "DackieSuperchain already deployed at %s", preComputedAddress, "on chain id: ", block.chainid
            );
            addr_ = preComputedAddress;
        } else {
            addr_ = address(new DackieSuperchain{salt: _implSalt()}(ownerAddr_, name, symbol, uint8(decimals)));
            console.log("Deployed DackieSuperchain at address: ", addr_, "on chain id: ", block.chainid);
        }
    }
